...
[![engine matched](https://img.shields.io/badge/engine%20matched-6.98%25-orange)](tools/progress.py)
...
> **Status: matching begins.** The ROM rebuilds byte-exact from split asm (`make verify` passes the SHA1), and the compiler is identified: engine code matches through SN64 GCC 2.8.1 `cc1 -O2` + `asn64`. 905 engine functions (6.98% of engine code) are matched and linked into the ROM as C so far ([docs/MATCHING.md](docs/MATCHING.md)); `tools/progress.py` tracks the share.
>
